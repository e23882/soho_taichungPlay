	...
	<div class="row"  style="margin-top:5%;margin-bottom:15%;width:100%; position: absolute;left: 0;right: 0;margin-left: auto;margin-right: auto;">
		...
		<?php
				...
				try
				{
					...
					$stmt = $conn->prepare("select id, name , type, 'attractions' as 'from' from attractions where name like '%".$_POST['rule']."%' UNION ALL select id, name , type, 'hotel' as 'from' from hotel where  name like '%".$_POST['rule']."%' UNION ALL select id, restName , restType , 'restaurant' as 'from' from restaurant  where restName like '%".$_POST['rule']."%'");
					...
					foreach ($result as $value) 
					{
						...
						if($value->from=="attractions")
						{
							echo "<div class='col-xs-6 col-md-3' style='text-align:center;background-color:rgba(255, 255, 255, 0.3);'>
						<a href='viewdetail.php?id=".$value->id."'><img src='images/hotel.png' style='border-radius:50%;width:60%;' /></a><br>
						...
						if($value->from=="restaurant")
						{
							echo "<div class='col-xs-6 col-md-3' style='text-align:center;background-color:rgba(255, 255, 255, 0.3);'>
						<a href='restdetail.php?id=".$value->id."'><img src='images/hotel.png' style='border-radius:50%;width:60%;' /></a><br>
						<span style='padding:5px;font-family:微軟正黑體;font-weight:bold;font-size:22px;'>".$value->name."</span></div>";
						}
